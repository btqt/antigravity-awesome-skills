...
System prompt đóng vai trò thiết lập nền tảng cho hành vi của LLM. Chúng định nghĩa role, mức độ chuyên môn, các ràng buộc và những kỳ vọng đối với output kết quả đầu ra.

## Cấu trúc System Prompt Hiệu quả (Effective System Prompt Structure)

```
[Role Definition] + [Expertise Areas] + [Behavioral Guidelines] + [Output Format] + [Constraints]
```

### Ví dụ: Code Assistant
...
## Thư viện Pattern (Pattern Library)
...
### Dynamic Role Adaptation (Thích ứng Role Thay đổi Linh hoạt)
...
### Constraint Specification (Khai báo Ràng buộc Đặc thù)
...
1. **Cụ thể hóa (Be Specific)**: Những role chung chung mơ hồ thường tạo ra các kiểu hành vi không nhất quán
2. **Thiết lập Boundaries (Set Boundaries)**: Xác định ranh giới định rõ cái gì model được và không được làm
3. **Cung cấp Ví dụ (Provide Examples)**: Mô phỏng hành vi mong muốn ngay bên trong system prompt
4. **Test Thoroughly (Kiểm thử Toàn vẹn)**: Xác minh system prompt đáp ứng hoạt động trơn tru qua nhiều input đầu vào đa dạng
5. **Iterate**: Liên tục tinh chỉnh lặp lại dựa trên các mô thức sử dụng (usage patterns) thực tế
6. **Version Control**: Theo dõi theo từng version đối với sự thay đổi của system prompt và performance

## Những Cạm bẫy Thường gặp (Common Pitfalls)

- **Quá dài (Too Long)**: Các system prompt lê thê quá mức làm phí phạm lượng tokens và làm loang lổ pha loãng sự tập trung
- **Quá mơ hồ (Too Vague)**: Các instruction phổ thông không uốn nắn hành vi được tác dụng hiệu quả
- **Các Chỉ dẫn Xung đột (Conflicting Instructions)**: Các nguyên tắc guideline trái nghịch lẫn nhau làm rối nháo model
- **Ràng buộc thái quá (Over-Constraining)**: Đẻ ra quá nhiều rule luật lệ có thể khiến các response mang tính khô cứng rigid
- **Chỉ thị Định dạng sơ sài (Under-Specifying Format)**: Thiếu hụt cấu trúc output dẫn đến thiếu tính đồng bộ nhất quán

## Kiểm thử System Prompts (Testing System Prompts)
