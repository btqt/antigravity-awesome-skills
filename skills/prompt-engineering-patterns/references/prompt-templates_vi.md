...
## Template Architecture (Kiến trúc Template)

### Basic Template Structure (Cấu trúc Template Cơ bản)
...
### Conditional Templates (Các Template có Điều kiện)
...
### Modular Template Composition (Cấu thành Template theo hướng Modular)
...
## Các Template Pattern Thường gặp (Common Template Patterns)

### Classification Template (Template Phân loại)
...
### Extraction Template (Template Trích xuất)
...
### Generation Template (Template Kiến tạo)
...
### Transformation Template (Template Chuyển đổi)
...
## Các Tính năng Chuyên sâu (Advanced Features)

### Template Inheritance (Sự Kế thừa Template)
...
### Variable Validation (Xác minh Biến số)
...
## Các Multi-Turn Templates

### Conversation Template (Template cho Cuộc hội thoại)
...
### State-Based Templates (Template dựa theo State)
...
1. **Keep It DRY (Đừng Tự Lặp lại Mình)**: Sử dụng các template để tránh sự lặp lại
2. **Validate Early (Kiểm chứng từ sớm)**: Rà soát các biến đầu vào (variables) trước quá trình rendering
3. **Version Templates**: Theo dấu mọi sự thay đổi hằng như code
4. **Test Variations (Kiểm thử Các biến thể)**: Đảm bảo các template hoạt động trơn tru bao trùm qua những dạng input phân rẽ đa hình
5. **Document Variables (Lưu hồ sơ cho các Biến)**: Chỉ định tường minh các biến bắt buộc/tuỳ chọn cần phải ngó tới
6. **Use Type Hints (Sử dụng các Type Hint)**: Định sẵn các variable types để dễ minh xác
7. **Provide Defaults (Cung cấp Giá trị Mặc định)**: Thiết lập giá trị mặc định có ý nghĩa khi phù hợp
8. **Cache Wisely (Cache khôn khéo)**: Vận Caching cho static template, chứ đừng dùng cho dạng hình dynamic

## Template Libraries (Thư viện Template)
...
### Content Generation (Khởi tạo Nội dung)
...
## Khía cạnh Performance (Performance Considerations)

- Pre-compile (Biên dịch trước) các templates cho các tái mục đích sử dụng thường nhật
- Vận dụng Cache cho rendered templates lúc các variables đều đang neo ở định mức static
- Quản lý co giảm triệt để khâu nối chuỗi (string concatenation) trong các vòng lập (loops)
- Nhắm tới sự hiệu quả của string formatting (cú pháp f-strings, thuật .format())
- Thiết lập Profile cho template rendering chặn trừ thắt cổ chai hiệu năng (bottlenecks)
